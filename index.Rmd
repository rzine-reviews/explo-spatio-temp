...
author: "Marion Le Texier (UMR 6266 IDEES)"
date: "`r Sys.Date()`"
output:
...
<p style="border:0.5px; border-style:solid; border-color:navajowhite3; padding: 1em; background-color:moccasin">
Cette fiche rend compte d'une série de traitements permettant d'explorer la [base de données MERIMEE](https://www.culture.gouv.fr/Espace-documentation/Base-de-donnees-Culture/Merimee-une-base-de-donnees-du-patrimoine-monumental-francais-de-la-Prehistoire-a-nos-jours){target="_blank"} qui consigne l'ensemble des bâtiments classés aux monuments historiques. Nous prendrons l'exemple des fiches "Mérimée" du patrimoine architectural toulousain disponibles sur le site [data.gouv.fr](https://www.data.gouv.fr/fr/datasets/base-merimee/){target="_blank"} 
<br> <br>
...
- `ggplot2` pour la représentation graphique. C'est une librairie de référence avec sa propre syntaxe
...
La fonction `installed.packages()` renseigne sur les packages déjà installés. Utilisons-la pour lister les packages que vous allez devoir installer.
...
Utilisons `install.packages()` pour installer les packages depuis le CRAN (*Comprehensive R Archive Network*). 
...
En appliquant `lapply()` sur le vecteur de noms de packages, nous chargeons (`library()`) toutes les librairies nécessaires 
...
<p class="center"> [<img src="figure/zip.png" width="50"/> **Télécharger les données**](data.zip) </p>  

<br>

**Deux couches géographiques en format** ***shape*** **sont mises à votre disposition** :

**1)** ***communes.shp*** :  Limites des communes de la métropole toulousaine récupérées sur [data.gouv.fr](https://www.data.gouv.fr/fr/datasets/communes/){target="_blank"} 
...
Les bâtiments toulousains classés aux monuments historiques les plus anciens datent de `r min(Merimee$chpdate)`, alors que les plus récents ont été construits en `r max(Merimee$chpdate)`, soit une période de `r max(Merimee$chpdate)-min(Merimee$chpdate)` ans. 
...
Il est intéressant de noter que le type d'objet *sf* est conservé malgré les importantes manipulations réalisées. Les géométries ont également été prises en compte. La variable *geometry* stocke désormais un ensemble de points (*MULTIPOINT*) décrivant la localisation de tous les bâtiments construits une même année.
...
**Dans un objectif comparatif, calculons des moyennes mobiles selon plusieurs valeurs de voisinage** ***k*** (5, 10, 20 et 40) et représentons différentes les courbes obtenues côte à côte.


Pour cela nous utiliserons la boucle `for()` qui permet de reproduire, en boucle, une série de traitement. 
...
for (k in c(5, 10, 20, 40)) {
  ...
  # Personalisation du nom avec la valeur de k
...
<p align="center">
$\hat{y_t}  = \alpha y_t + \alpha (1-\alpha)y_{t-1} + (1-\alpha)^2 y_{t-2} + (1-\alpha)^3 y_{t-3} + ... etc.$    
<p/>
...
La fonction `ses()` (*simple exponential smoothing*) de la librairie `forecast` permet d'ajuster un modèle exponentiel simple à sa série statistique. **L'ajustement consiste à déterminer la valeur du coefficient de lissage $\alpha$**.
...
Commençons par créer la variable qui contient le texte à afficher dans le fenêtre interactive.
...
Merimee_map <- leaflet() %>%
               addTiles() %>%
               addMarkers(lng = unlist(map(Merimee$geometry,1)),
...
Si l'on ne souhaite pas estimer des valeurs d'intensité de présence (ce qui nécessite de définir quels sont le voisinage, le type de distance, et la fonction d’impédance de cette dernière les plus opportuns à considérer dans le cas étudié), une solution simple consiste à subdiviser l'espace d'étude par une grille régulière (carroyage) et de comptabiliser le nombre de monuments localisés dans chaque carreau.
...
# Construction de la grille régulière à partir de l'étendue géographique des données
Merimee_grid <- raster(extent(Merimee),
                       crs = st_crs(Merimee)$proj4string, 
                       nrow=50, 
                       ncol=50)
...
Il s'agit d'une variable quantitative absolue, mais nous pouvons interpréter cette variable comme une densité (variable quantitative relative) car les valeurs sont calculées sur des surfaces identiques. 
...
Les méthodes de groupement d’évènements en fonction de leur proximité spatio-temporelle sont nombreuses [@Spatiotemp_cluster].

@STDBSCAN **proposent d'étendre la méthode de l'algorithme de détection de** ***clusters*** **à partir de l'analyse des densités DBSCAN aux bases de données spatio-temporelles**. Ce nouvel algorithme, nommé ST-DBSCAN, reprend la logique du premier : balayer l'espace, compter pour chaque objet le nombre de voisins localisés dans une région d'un certain rayon autour de lui, et considérer que les objets contenus dans une région bénéficiant d'une densité supérieure à un certain seuil appartiennent à un même cluster, à ceci près qu'il introduit également un critère de proximité temporelle. 
...
**Exécuter le code**. Vous pouvez désormais utiliser cette fonction de partitionnement comme n'importe qu'elle autre fonction.
...
```{r session_info, eval=TRUE, echo=FALSE, warning=FALSE}
